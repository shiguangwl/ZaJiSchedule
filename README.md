...
#### 2. 智能 CPU 调度算法
- **滚动窗口算法**: 固定窗口开始时间（如每天00:00），窗口内动态调度，窗口切换时自动重置
- **最低负载预留**: 优先保证剩余时间的最低负载配额（如10%），剩余配额动态分配
...
### Q: 为什么窗口开始时 CPU 限制会降低？
A: 这是滚动窗口机制。窗口开始时（如每天00:00），窗口重置，剩余配额恢复到总配额，系统会根据剩余配额重新计算安全限制。随着窗口内数据积累，限制会动态调整
