...
class CPUScheduler:
    ...
    def _get_current_window_bounds(self) -> tuple[datetime, datetime]:
        ...
        """
        now = datetime.now()
        window_start_hour = self.config.window_start_hour
    ...
    def calculate_rolling_window_avg(self) -> tuple[float, int]:
        ...
        total_cpu = sum(m["cpu_percent"] for m in window_metrics)
        avg_cpu = total_cpu / len(window_metrics)
    ...
    def calculate_safe_cpu_limit(self) -> float:
        ...
        logger.info(
            f"[滚动窗口] 窗口: {window_start.strftime('%H:%M')}-{window_end.strftime('%H:%M')}, "
            f"已用时间: {elapsed_minutes:.1f}min, 剩余时间: {remaining_minutes:.1f}min, "
            f"平均CPU: {avg_cpu:.2f}%, 剩余配额: {remaining_quota:.2f}%·min",
        )

        # 如果剩余时间很少（< 1分钟），使用最低负载
        if remaining_minutes < 1.0:
            safe_limit = min_load
            logger.info(f"[滚动窗口] 窗口即将结束，使用最低负载: {safe_limit:.2f}%")
        else:
            # 计算未来需要的最低负载配额
            ...
            if remaining_quota <= 0:
                ...
                safe_limit = min_load
                logger.error(
                    f"[滚动窗口] ⚠️ 配额管理失败！剩余配额耗尽: {remaining_quota:.2f}%·min <= 0, "
                ...
                safe_limit = min_load
                logger.error(
                    f"[滚动窗口] ⚠️ 配额管理失败！剩余配额不足: {remaining_quota:.2f}%·min < "
                ...
                logger.info(
                    f"[滚动窗口] 配额充足: remaining_quota={remaining_quota:.2f}%·min, "
                    f"future_min_quota={future_min_quota:.2f}%·min, "
        ...
        logger.info(f"[滚动窗口] 最终限制: {safe_limit:.2f}%")
    ...
    def get_scheduler_status(self) -> dict[str, Any]:
        ...
        startup_threshold = expected_data_points * (self.config.startup_data_threshold_percent / 100)
        is_startup_period = data_points < startup_threshold

        return {
            "safe_cpu_limit": round(safe_limit, 2),
            "current_cpu_percent": round(current_cpu, 2),
            "rolling_window_avg_cpu": round(avg_cpu, 2),
